...
export function StockReservationsPage() {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    try {
      const [stockRequests, snapshot, taskAssignees, requestNumberPreview] = await Promise.all([fetchStockRequests(), fetchInventorySnapshot(), fetchTaskAssignees(), fetchNextStockRequestNumber()]);
      setRequests(stockRequests); setProducts(snapshot.products); setAssignees(taskAssignees); setNextRequestNumber(requestNumberPreview.requestNumber); setLastUpdatedAt(new Date());
    } catch (error) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if (!nextRequestNumber) throw new Error("Wait for the request number to load, then confirm again.");
      const createdRequest = await createStockRequest({ requestType: form.requestType, expectedRequestNumber: nextRequestNumber, requestedFor: form.requestedFor, requiredDate: new Date(`${form.requiredDate}T17:00:00`).toISOString(), notes: form.notes || undefined, lines: [{ productId: form.productId, requiredQuantity: Number(form.quantity) }] });
      setForm({ requestType: "CUSTOMER_ORDER", requestedFor: "", requiredDate: today, productId: "", quantity: "", notes: "" });
      setShowForm(false); setMessage(`Stock request ${createdRequest.requestNumber} confirmed. It is ready for reservation.`); await refresh();
    } catch (error) { setMessage(error instanceof Error ? error.message : "The stock request could not be saved."); await refresh(true); } finally { setBusyId(null); }
  }
  ...
  return <section id="manager-reservations" className="space-y-5">
    ...
    {showForm && <form onSubmit={submit} className="grid gap-4 rounded-[28px] border border-[#dce7f5] bg-white p-6 md:grid-cols-2 xl:grid-cols-4">
      ...
      <Field label="Order / request number"><div className="input flex items-center gap-2 bg-[#f7f9fc] font-extrabold text-[#17345f]" aria-label={`Order or request number ${nextRequestNumber || "loading"}`}><LockKeyhole size={14} className="text-[#2f5d9c]" />{nextRequestNumber || "Loading number…"}</div></Field>
      ...
      <button disabled={busyId === "create" || !nextRequestNumber} className="self-end rounded-xl bg-[#1763f6] px-5 py-3 font-extrabold text-white disabled:opacity-50">{busyId === "create" ? "Saving..." : "Confirm request"}</button>
    </form>}
