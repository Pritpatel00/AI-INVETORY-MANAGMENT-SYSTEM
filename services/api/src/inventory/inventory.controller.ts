...
import { NotificationQueueService } from "../notifications/notifications.service";
...
export class InventoryController {
  ...
  @Post("reorder-drafts/:id/approve")
  @Roles("manager", "administrator")
  @ApiOperation({ summary: "Approve a reorder draft." })
  approveReorderDraft(
    @Param("id", new ParseUUIDPipe()) id: string,
    @Body() input: ReorderDecisionDto,
    @Req() request: AuthenticatedRequest,
  ) {
    return this.inventoryService.approveReorderDraft(
      id,
      input,
      request.authUser!,
    );
  ...
  @Post("reorder-drafts/:id/cancel")
  @Roles("manager", "administrator")
  @ApiOperation({ summary: "Cancel a reorder draft." })
  cancelReorderDraft(
    @Param("id", new ParseUUIDPipe()) id: string,
    @Body() input: ReorderDecisionDto,
    @Req() request: AuthenticatedRequest,
  ) {
    return this.inventoryService.cancelReorderDraft(
      id,
      input,
      request.authUser!,
    );
  ...
  @Post("reorder-drafts/:id/queue-email")
  @Roles("manager", "administrator")
  @ApiOperation({ summary: "Queue the supplier email for an approved reorder draft." })
  queueReorderEmail(@Param("id", new ParseUUIDPipe()) id: string) {
    return this.notifications.enqueueReorderEmail(id, false);
  }

  @Post("reorder-drafts/:id/retry-email")
  @Roles("manager", "administrator")
  @ApiOperation({ summary: "Retry a failed supplier email delivery." })
  retryReorderEmail(@Param("id", new ParseUUIDPipe()) id: string) {
    return this.notifications.enqueueReorderEmail(id, true);
  }
